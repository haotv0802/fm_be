 ...
 */
@Service("paymentMethodUpdateValidator")
public class PaymentMethodUpdateValidator implements Validator<PaymentMethodPresenter> {

    private final IPaymentMethodsDao paymentMethodsDao;

    @Autowired
    public PaymentMethodUpdateValidator(@Qualifier("paymentMethodsDao") IPaymentMethodsDao paymentMethodsDao) {
    ...
    public void validate(PaymentMethodPresenter paymentMethod, String faultCode, Object... args) {
        ...
        Assert.notNull(paymentMethod.getId());
        Assert.notNull(paymentMethod.getName());

        if (paymentMethodsDao.isPaymentNameExisting(paymentMethod.getId(), paymentMethod.getName())) {
