        ...
        try {
            ...
            for (int i = 1; i <= limitPageNum; i++) {
                ...
                for (int y = 0; y < subJsonArray.length(); y++) {
                    ...
                    String url = subJsonArray.getJSONObject(y).getString("get_permalinks");
                    PromotionCrawlerModel model = new PromotionCrawlerModel(title, description, promotionUtils.getProvision(description), null,
                            StringUtils.isEmpty(date_start) ? LocalDate.now() : FmLocalDateUtils.parseDateWithPattern(date_start, "dd-MM-yyyy"),
                            StringUtils.isEmpty(date_end) ? FmLocalDateUtils.getLastDateOfNextYear() : FmLocalDateUtils.parseDateWithPattern(date_end, "dd-MM-yyyy"),
                            cateId, bankId, contentHTML, url, null, null, null, null);
                    list.add(model);
                }
            }
            return list;
