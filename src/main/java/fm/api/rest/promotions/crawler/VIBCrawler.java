...
public class VIBCrawler implements IBankPromotionCrawler {
  ...
  private final int PROMOTION_CRAWLING_OPTION = 1;
  private final int INSTALLMENT_CRAWLING_OPTION = 2;
...
//            e.printStackTrace();
   ...
   * @throws InterruptedException
   */
  private List<PromotionCrawlerModel> doCrawling(int cateID, String url, int crawling_option) throws InterruptedException {
    ...
    for (String link : linkPromotionsLinks) {
      ...
      PromotionCrawlerModel model = getPromotionFromLink(link, cateID, crawling_option);
      if (model != null) {
