...
public class ConnectionsWatchdog implements Runnable {
    private static final Logger logger = LogManager.getLogger(ConnectionsWatchdog.class);
    ...
    public long checkConnections(Iterator<TrackingConnectionWrapper> it) {
        ...
        while (it.hasNext()) {
            ...
            logger.info("maxTimeoutInterval " + maxTimeoutInterval);

            if (timePassed > maxTimeoutInterval) {
                logger.info("removing expired connection " + conn.getTransactionId());
                it.remove();
                tryRollback(conn);
                tryClose(conn);
                // TODO: release the connection (return to the datasource)
