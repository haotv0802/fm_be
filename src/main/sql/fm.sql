...
CREATE TABLE `fm_expenses` (
  ...
  `event_type_id` BIGINT      NULL, # in case it is an event => is_an_event = true
  `event_name`    VARCHAR(45) NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `fm_expenses_id_unique` (`id`),
  CONSTRAINT `fm_expenses_user_id` FOREIGN KEY (`user_id`) REFERENCES `fm_users` (`id`),
  CONSTRAINT `fm_expenses_event_type_id` FOREIGN KEY (`event_type_id`) REFERENCES `fm_event_types` (`id`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
